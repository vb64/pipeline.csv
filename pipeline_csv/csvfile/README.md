# Формат CSV файла с данными о трубопроводе
...
- 0 Поперечный шов
- 1 Линейный обьект / маркер
- 2 Дефект
- 3 Переход (изменение) толщины
- 4 Продольный шов
...
Колонка '03' Object_Code может содержать пользовательский код обьекта.

### Название обьекта

Колонка '05' Object_Code_T может содержать текстовое пользовательское название обьекта, например 'Прямошовная секция'.
...
Если значение колонки 02 TypeObject установлено в 0 (Поперечный шов)

Колонка 04 ObjectName может содержать пользовательский номер трубы (произвольная строка).

Колонка 09 Depth_min может содержать радиус изгиба трубы в мм (строка).
...
Если значение колонки 02 TypeObject установлено в 1 (Линейный обьект / маркер).

Колонка 03 Object_Code должна содержать код объекта согласно пользовательской классификации.

Например: 0 (Кран), 1 (Тройник) и т.д.
...
Например: 0 (Коррозия), 1 (Механическое повреждение) и т.д.
...
- 15 Type_Def Положение дефекта на стенке трубы

Положение дефекта на стенке трубы должно быть одним из следующих:
...
### Переход (изменение) диаметра

Если значение колонки 02 TypeObject установлено в 6 (Переход (изменение) диаметра)
