...
Рабочая версия примера онлайн-калькулятора опасности дефекта с использованием этой библиотеки [находится здесь](https://clan-panel-na.appspot.com/).
Исходный код примера содержится в [каталоге example](/example/integrity/web/gae/) репозитария.

Исходный код примера онлайн-калькулятора для запуска на локальном компьютере находится в [каталоге localhost](/example/integrity/web/localhost/) репозитария.
